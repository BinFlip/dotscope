...
pub fn register(manager: &HookManager) {
    ...
    // File.Exists — emulated environment has no filesystem
    manager.register(
...
/// Hook for `System.IO.File.Exists(string) -> bool`.
///
/// Returns `true` — the emulated environment pretends files exist so that
/// code paths that check for file existence proceed normally. This allows
/// anti-tamper checks to continue to the hash verification stage where the
/// VerifyHash bypass handles the actual verification.
fn file_exists_pre(_ctx: &HookContext<'_>, _thread: &mut EmulationThread) -> PreHookResult {
