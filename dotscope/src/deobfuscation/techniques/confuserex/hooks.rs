...
/// # Arguments
///
/// * `tokens` - Set of protection method tokens to stub out.
///
/// # Returns
///
...
fn is_lzma_input(ctx: &HookContext<'_>, thread: &EmulationThread) -> bool {
    ...
    for (idx, arg) in ctx.args.iter().enumerate() {
        ...
        // Try to get array data from heap — both methods return Result<Option<...>>,
        // so propagate errors and fall through on None.
        let byte_data = match thread
            .heap()
            .get_array_as_bytes(*heap_ref, ctx.pointer_size)
        {
            Ok(Some(data)) => data,
            Ok(None) | Err(_) => match thread.heap().get_byte_array(*heap_ref) {
                Ok(Some(data)) => data,
...
mod tests {
    ...
    use crate::{
        deobfuscation::techniques::confuserex::hooks::{
            create_anti_tamper_stub_hook, create_lzma_hook,
        },
    ...
    #[test]
    fn test_create_anti_tamper_stub_hook() {
        let mut tokens = HashSet::new();
        tokens.insert(Token::new(0x06000001));
        tokens.insert(Token::new(0x06000002));

        let hook = create_anti_tamper_stub_hook(tokens);
        assert_eq!(hook.name(), "confuserex-protection-stub");
        assert_eq!(hook.priority(), HookPriority::HIGHEST);
    }
}
