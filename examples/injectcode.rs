...
mod tests {
    ...
    fn test_signature_creation() {
        let signature = create_writeline_signature().unwrap();
        assert!(!signature.is_empty());

        // Basic signature validation - should start with calling convention flags
        assert_eq!(signature[0] & 0x0F, 0x00); // Default calling convention
    }

    #[test]
    fn test_user_string_token_construction() {
        let index = 42u32;
        let token = Token::new(0x70000000 | index);

        // Verify it's a UserString token
        assert_eq!(token.table(), 0x70);
        assert_eq!(token.row(), index);
    }

    #[test]
    fn test_mscorlib_ref_creation() {
        // This would require a test assembly, but we can test the builder pattern
        let _builder = AssemblyRefBuilder::new()
            .name("System.Runtime")
            .version(8, 0, 0, 0)
    ...
    #[test]
    fn test_console_typeref_creation() {
        let _builder = TypeRefBuilder::new().name("Console").namespace("System");
