...
use crate::{
    ...
    metadata::{
        method::Method, signatures::TypeSignature, token::Token, typesystem::CilTypeReference,
    },
    prelude::FlowType,
    CilObject,
};
    ...
    for instr in method.instructions() {
        ...
        if token.table() == 0x0A {
            if let Some(member_ref) = assembly.refs_members().get(token) {
                ...
                let type_name = get_type_ref_name(&member.declaredby);
...
fn get_type_ref_name(type_ref: &CilTypeReference) -> String {
    match type_ref {
        CilTypeReference::TypeRef(tr) => tr.name().unwrap_or_default(),
        CilTypeReference::TypeDef(td) => td.name().unwrap_or_default(),
        CilTypeReference::TypeSpec(ts) => ts.name().unwrap_or_default(),
        _ => String::new(),
    }
}
