...
mod tests {
    ...
    fn create_assembly_with_broken_dependency_chain() -> Result<NamedTempFile> {
        let Some(clean_testfile) = get_clean_testfile() else {
    ...
    fn create_assembly_with_unsatisfied_transitive_dependencies() -> Result<NamedTempFile> {
        let Some(clean_testfile) = get_clean_testfile() else {
            return Err(Error::Error("WindowsBase.dll not available".to_string()));
        };
    ...
    fn create_assembly_with_invalid_dependency_ordering() -> Result<NamedTempFile> {
        let Some(clean_testfile) = get_clean_testfile() else {
            return Err(Error::Error("WindowsBase.dll not available".to_string()));
        };
    ...
    fn create_assembly_with_self_referential_dependencies() -> Result<NamedTempFile> {
        let Some(clean_testfile) = get_clean_testfile() else {
            return Err(Error::Error("WindowsBase.dll not available".to_string()));
        };
