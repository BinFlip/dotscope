...
//! # Key Components
//!
//! - [`crate::metadata::validation::validators::owned::constraints::type::OwnedTypeConstraintValidator`] - Main validator implementation providing comprehensive constraint validation
//!
//! # Usage Examples
//!
//! ```rust,no_run
...
impl OwnedTypeConstraintValidator {
    ...
    /// # Returns
    ///
    /// A new [`crate::metadata::validation::validators::owned::constraints::type::OwnedTypeConstraintValidator`] instance ready for validation operations.
    ///
    /// # Thread Safety
    ///
...
mod tests {
    ...
    #[test]
    fn test_owned_type_constraint_validator_creation() {
        let validator = OwnedTypeConstraintValidator::new();
        assert_eq!(validator.name(), "OwnedTypeConstraintValidator");
        assert_eq!(validator.priority(), 185);
    }

    #[test]
    fn test_owned_type_constraint_validator_should_run() {
    ...
    #[test]
    fn test_owned_type_constraint_validator_validate_on_real_assembly() {
        // Test that validator works on a real assembly (should not find constraint issues)
        let path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/samples/WindowsBase.dll");
        if let Ok(view) = CilAssemblyView::from_file(&path) {
            if let Ok(object) = CilObject::from_file(&path) {
                ...
                // WindowsBase.dll should have valid constraints
                let result = validator.validate_owned(&context);
                assert!(
                    result.is_ok(),
                    "WindowsBase.dll should have valid constraints: {result:?}"
                );
            }
        }
    }
