...
mod tests {
    ...
    #[test]
    fn test_owned_type_dependency_validator_creation() {
        let validator = OwnedTypeDependencyValidator::new();
        assert_eq!(validator.name(), "OwnedTypeDependencyValidator");
        assert_eq!(validator.priority(), 170);
    }

    #[test]
    fn test_owned_type_dependency_validator_should_run() {
    ...
    #[test]
    fn test_owned_type_dependency_validator_validate_on_real_assembly() {
        // Test that validator works on a real assembly (should not find dependency issues)
        let path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/samples/WindowsBase.dll");
        if let Ok(view) = CilAssemblyView::from_file(&path) {
            if let Ok(object) = CilObject::from_file(&path) {
                ...
                // WindowsBase.dll should have valid dependencies
                let result = validator.validate_owned(&context);
                assert!(
                    result.is_ok(),
                    "WindowsBase.dll should have valid dependencies: {result:?}"
                );
            }
        }
    }
    ...
    #[test]
    fn test_owned_type_dependency_validator_validate_disabled() {
        ...
        if let Ok(view) = CilAssemblyView::from_file(&path) {
            if let Ok(object) = CilObject::from_file(&path) {
                ...
                // But if we call validate_owned directly, it should succeed (validation performed)
                let result = validator.validate_owned(&context);
                assert!(
                    result.is_ok(),
                    "Validation should succeed even when disabled: {result:?}"
                );
            }
        }
    }
    ...
    #[test]
    fn test_type_dependency_resolution_validation() {
        // Test the logic for type dependency resolution validation
        let path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/samples/WindowsBase.dll");
        if let Ok(view) = CilAssemblyView::from_file(&path) {
            if let Ok(object) = CilObject::from_file(&path) {
                let config = ValidationConfig {
                ...
                // Test the private method logic (indirectly through validate_owned)
                let result = validator.validate_type_dependency_resolution(&context);
                assert!(
                    result.is_ok(),
                    "Type dependency resolution should succeed: {result:?}"
                );
            }
        }
    }
    ...
    #[test]
    fn test_method_signature_dependencies_validation() {
        // Test method signature dependency validation
        let path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/samples/WindowsBase.dll");
        if let Ok(view) = CilAssemblyView::from_file(&path) {
            if let Ok(object) = CilObject::from_file(&path) {
                let config = ValidationConfig {
                ...
                // Test method signature dependency validation
                let result = validator.validate_method_signature_dependencies(&context);
                assert!(
                    result.is_ok(),
                    "Method signature dependencies should be valid: {result:?}"
                );
            }
        }
    }
    ...
    #[test]
    fn test_dependency_path_accessibility_validation() {
        // Test dependency path accessibility validation
        let path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/samples/WindowsBase.dll");
        if let Ok(view) = CilAssemblyView::from_file(&path) {
            if let Ok(object) = CilObject::from_file(&path) {
                let config = ValidationConfig {
                ...
                // Test dependency path accessibility validation
                let result = validator.validate_dependency_path_accessibility(&context);
                assert!(
                    result.is_ok(),
                    "Dependency paths should be accessible: {result:?}"
                );
            }
        }
    }
