...
mod tests {
    ...
    fn create_assembly_with_invalid_security_action() -> Result<NamedTempFile> {
        let Some(clean_testfile) = get_clean_testfile() else {
    ...
    fn create_assembly_with_malformed_permission_set() -> Result<NamedTempFile> {
        let Some(clean_testfile) = get_clean_testfile() else {
            return Err(Error::Error("WindowsBase.dll not available".to_string()));
        };
    ...
    fn create_assembly_with_conflicting_security_attributes() -> Result<NamedTempFile> {
        let Some(clean_testfile) = get_clean_testfile() else {
            return Err(Error::Error("WindowsBase.dll not available".to_string()));
        };
    ...
    fn create_assembly_with_security_transparency_violations() -> Result<NamedTempFile> {
        let Some(clean_testfile) = get_clean_testfile() else {
            return Err(Error::Error("WindowsBase.dll not available".to_string()));
        };
