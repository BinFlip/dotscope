...
use dotscope::assembly::{InstructionAssembler, InstructionEncoder, decode_instruction, decode_stream};
...
pub fn criterion_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("bench_assemble_complex_method", |b| {
        b.iter(|| {
            let mut asm = InstructionAssembler::new();
            asm.ldc_i4_0().unwrap()                    // int i = 0
                .stloc_0().unwrap()
                .br("loop_condition").unwrap()
                .label("loop_start").unwrap()
                .ldarg_0().unwrap()                    // Load array
                .ldloc_0().unwrap()                    // Load index
                .ldarg_1().unwrap()                    // Load value
                .stelem_i4().unwrap()                  // array[i] = value
                .ldloc_0().unwrap()                    // i++
                .ldc_i4_1().unwrap()
                .add().unwrap()
                .stloc_0().unwrap()
                .label("loop_condition").unwrap()
                .ldloc_0().unwrap()                    // if (i < 10)
                .ldc_i4_const(10).unwrap()
                .clt().unwrap()
                .brtrue("loop_start").unwrap()
                .ret().unwrap();
            asm.finish().unwrap()
        });
    });
    ...
    c.bench_function("bench_assemble_object_method", |b| {
        ...
        b.iter(|| {
            let mut asm = InstructionAssembler::new();
            asm.ldarg_0().unwrap()                     // this
                .ldfld(field_token).unwrap()           // Load field
                .ldnull().unwrap()                     // Compare with null
                .ceq().unwrap()
                .brfalse("not_null").unwrap()
                .ldarg_0().unwrap()                    // Create new object
                .newobj(method_token).unwrap()
                .stfld(field_token).unwrap()
                .label("not_null").unwrap()
                .ldarg_0().unwrap()                    // Return field value
                .ldfld(field_token).unwrap()
                .callvirt(method_token).unwrap()
                .ret().unwrap();
            asm.finish().unwrap()
        });
    });
    ...
    c.bench_function("bench_assemble_with_optimizations", |b| {
        b.iter(|| {
            let mut asm = InstructionAssembler::new();
            asm.ldc_i4_const(0).unwrap()      // Should use ldc.i4.0
                .ldc_i4_const(1).unwrap()     // Should use ldc.i4.1
                .ldc_i4_const(127).unwrap()   // Should use ldc.i4.s
                .ldc_i4_const(1000).unwrap()  // Should use ldc.i4
                .add().unwrap()
                .add().unwrap()
                .add().unwrap()
                .ret().unwrap();
    ...
    c.bench_function("bench_assemble_large_method", |b| {
        b.iter(|| {
            ...
            for i in 0..50 {
                asm.ldarg_0().unwrap()
                    .ldc_i4_const(i).unwrap()
                    .ceq().unwrap()
                    .brtrue(&format!("case_{}", i)).unwrap();
            }
            
            asm.ldc_i4_m1().unwrap()
                .ret().unwrap();
            
            for i in 0..50 {
                asm.label(&format!("case_{}", i)).unwrap()
                    .ldc_i4_const(i * 2).unwrap()
                    .ret().unwrap();
            }
